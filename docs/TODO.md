...
_Last updated: 2026-04-24 — W3.2l (`morocco_dental_expo`) shipped end-to-end; GitHub Actions workflows were migrated onto Node 24-capable action versions with an explicit `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24` opt-in; mainline now has eight curated sources, Playwright option D, and clean DB-test hygiene. The only required next step is still W3.2d live Fly deploy, and the blocker remains Fly billing/payment setup._
