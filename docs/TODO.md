...
_Last updated: 2026-04-24 — W3.2m (`europerio`) shipped end-to-end; W3.2l (`morocco_dental_expo`) and the GitHub Actions Node 24 migration remain on main; mainline now has nine curated sources, Playwright option D, and clean DB-test hygiene. The only required next step is still W3.2d live Fly deploy, and the blocker remains Fly billing/payment setup._
