...
_Last updated: 2026-04-24 — W3.2o (`forum_officine_tn`) shipped end-to-end; the Node 24 workflow migration remains on main; Playwright option D is wired in repo workflows; DB-gated ingest suites use `TEST_DATABASE_URL`. Main ingests eleven curated sources with operator-safe preview mode. W3.2d live deploy remains the only required operator-gated blocker, and the concrete stop is still Fly billing setup._

## Status

**W0+W1 foundation + W2 ADA + W3.1 GNYDM + W3.2a bookkeeping + W3.2b scheduler-primitive CLI + W3.2c drift observability + provenance + W3.2e third source (AAP Annual Meeting) + W3.2f preview mode + W3.2g ADA silent-drop aggregate review items + W3.2h fourth source (`fdi_wdc`) + W3.2i fifth source (`eao_congress`) + W3.2j sixth source (`cds_midwinter`) + W3.2k seventh source (`dentex_algeria`) + W3.2l eighth source (`morocco_dental_expo`) + W3.2m ninth source (`europerio`) + W3.2n tenth source (`amied_congress`) + W3.2o eleventh source (`forum_officine_tn`) + the GitHub Actions Node 24 migration are all complete.** Ingest exposes `medevents-ingest run --all [--force] [--dry-run]` with belt-and-braces read-only safety; operator can preview any real run against production with zero DB writes. The ADA parser now emits a drift signal (`ParserReviewRequest` → `parser_failure` review_item) if any schedule row silently drops — closing the W2 spec §7 observability gap. Admin UI shows real timestamps from W3.2a. Repo workflows now opt into GitHub's Node 24 runtime with current action majors in `CI` and `Nightly smoke`, removing the previously tracked Node 20 deprecation follow-up. See [`docs/runbooks/w3.2o-done-confirmation.md`](runbooks/w3.2o-done-confirmation.md) and predecessors for evidence maps.
...
- Live Morocco Dental Expo smoke completed on 2026-04-24: first run `source=morocco_dental_expo fetched=2 skipped_unchanged=0 created=1 updated=1 review_items=0`; two final reruns both reported `fetched=2 skipped_unchanged=2 created=0 updated=0`; dev DB holds one `Morocco Dental Expo 2026` row with two `event_sources` rows, canonical English-homepage `source_url`, `venue_name = ICEC AIN SEBAA`, and populated source bookkeeping. Full ingest suite now at `180 passed`.
- Live EuroPerio smoke completed on 2026-04-24: first run `source=europerio fetched=2 skipped_unchanged=0 created=1 updated=1 review_items=0`; rerun `fetched=2 skipped_unchanged=2 created=0 updated=0`; dev DB holds one `EuroPerio12` row with two `event_sources` rows, canonical detail-page `source_url`, and populated source bookkeeping. Full ingest suite now at `188 passed`.
- Live AMIED smoke completed on 2026-04-24: first run `source=amied_congress fetched=2 skipped_unchanged=0 created=1 updated=1 review_items=0`; rerun `fetched=2 skipped_unchanged=2 created=0 updated=0`; dev DB holds one `AMIED International Congress 2026` row with two `event_sources` rows, canonical homepage `source_url`, and populated source bookkeeping. Full ingest suite now at `196 passed`.
- Live Forum de l'Officine smoke completed on 2026-04-24: first run `source=forum_officine_tn fetched=2 skipped_unchanged=0 created=1 updated=1 review_items=0`; rerun `fetched=2 skipped_unchanged=2 created=0 updated=0`; dev DB holds one `Forum de l'Officine 2026` row with two `event_sources` rows, canonical homepage `source_url`, live public `registration_url`, and populated source bookkeeping. Full ingest suite now at `204 passed`.
...
_None. W3.2o shipped; the next optional wave is to be chosen in `docs/TODO.md`._

Historical wave sub-specs:

- W3.2o: [`docs/superpowers/specs/2026-04-24-medevents-w3-2o-forum-officine-tn.md`](superpowers/specs/2026-04-24-medevents-w3-2o-forum-officine-tn.md) — shipped (see [`docs/runbooks/w3.2o-done-confirmation.md`](runbooks/w3.2o-done-confirmation.md))
- W3.2n: [`docs/superpowers/specs/2026-04-24-medevents-w3-2n-amied-congress.md`](superpowers/specs/2026-04-24-medevents-w3-2n-amied-congress.md) — shipped (see [`docs/runbooks/w3.2n-done-confirmation.md`](runbooks/w3.2n-done-confirmation.md))
- W3.2m: [`docs/superpowers/specs/2026-04-24-medevents-w3-2m-europerio.md`](superpowers/specs/2026-04-24-medevents-w3-2m-europerio.md) — shipped (see [`docs/runbooks/w3.2m-done-confirmation.md`](runbooks/w3.2m-done-confirmation.md))
- W3.2l: [`docs/superpowers/specs/2026-04-24-medevents-w3-2l-morocco-dental-expo.md`](superpowers/specs/2026-04-24-medevents-w3-2l-morocco-dental-expo.md) — shipped (see [`docs/runbooks/w3.2l-done-confirmation.md`](runbooks/w3.2l-done-confirmation.md))
...
- [`docs/runbooks/morocco-dental-expo-fixtures.md`](runbooks/morocco-dental-expo-fixtures.md) — Morocco Dental Expo homepage/exhibitor-list fixtures + ASP.NET hidden-field normalization rationale
- [`docs/runbooks/amied-fixtures.md`](runbooks/amied-fixtures.md) — AMIED homepage/inscriptions fixtures + sanitized registration artifact rationale
- [`docs/runbooks/forum-officine-fixtures.md`](runbooks/forum-officine-fixtures.md) — Forum de l'Officine homepage/practical-info fixtures + assistant-widget date-conflict rationale
...
- W3.1 through W3.2c shipped on 2026-04-22/23; W3.2e (third source AAP), W3.2f (`--dry-run`), W3.2g (ADA silent-drop review items), DB-test hygiene, Playwright option D, W3.2h (`fdi_wdc`), W3.2i (`eao_congress`), W3.2j (`cds_midwinter`), W3.2k (`dentex_algeria`), W3.2l (`morocco_dental_expo`), W3.2m (`europerio`), W3.2n (`amied_congress`), and W3.2o (`forum_officine_tn`) all shipped by 2026-04-24. W3.2d (Fly scheduled machines) has repo artifacts on `main`, and live deploy attempts on 2026-04-23 and 2026-04-24 confirmed the remaining blocker is Fly billing/payment setup: `fly auth login` works, but `fly apps create medevents-ingest` stops at Fly's payment gate. Eleven curated sources now run locally via `run --all` with operator-safe `--dry-run` preview. There is no remaining user-gated queue item.
- Local dev DB now holds: 6 ADA events (W2 smoke) + 3 GNYDM editions (W3.1 smoke) + 1 FDI row + 3 EAO congress rows + 1 CDS Midwinter row + 1 Dentex Algeria row + 1 Morocco Dental Expo row + 1 EuroPerio row + 1 AMIED row + 1 Forum de l'Officine row. The 2026 GNYDM, FDI 2026, EAO 2026, CDS Midwinter 2026, Dentex Algeria 2026, Morocco Dental Expo 2026, EuroPerio12, AMIED International Congress 2026, and Forum de l'Officine 2026 rows each have 2 `event_sources` rows. That state is safe to keep.
...
| W3.2l — eighth curated source (`morocco_dental_expo`)                                            | ✅ Complete — parser + 9 tests (7 unit + 2 DB-gated), config seeded, live smoke verified on the official English homepage + public exhibitor-list page (1 event row total; canonical row keeps the English homepage as `source_url`; final unchanged reruns = skipped_unchanged=2 after ASP.NET hidden-field hash normalization). Full ingest suite at 180 passed. See w3.2l-done-confirmation.md.  |
| W3.2m — ninth curated source (`europerio`)                                                       | ✅ Complete — parser + 8 tests (6 unit + 2 DB-gated), config seeded, live smoke verified on the official EFP EuroPerio hub + EuroPerio12 detail page (1 event row total; canonical row keeps the detail page as `source_url`; unchanged rerun = skipped_unchanged=2 with raw-body hashing). Full ingest suite at 188 passed. See w3.2m-done-confirmation.md.                                        |
| W3.2n — tenth curated source (`amied_congress`)                                                  | ✅ Complete — parser + 8 tests (6 unit + 2 DB-gated), config seeded, live smoke verified on the official AMIED homepage + inscriptions page (1 event row total; canonical row keeps the homepage as `source_url`; unchanged rerun = skipped_unchanged=2 with raw-body hashing). Full ingest suite at 196 passed. See w3.2n-done-confirmation.md.                                                    |
| W3.2o — eleventh curated source (`forum_officine_tn`)                                            | ✅ Complete — parser + 8 tests (6 unit + 2 DB-gated), config seeded, live smoke verified on the official homepage + practical-information page (1 event row total; canonical row keeps the homepage as `source_url`; unchanged rerun = skipped_unchanged=2 with raw-body hashing despite the stale assistant-widget May 1-2 data). Full ingest suite at 204 passed. See w3.2o-done-confirmation.md. |
| CI/runtime hygiene — GitHub Actions Node 24 migration                                            | ✅ Complete — `CI` and `Nightly smoke` now set `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24: "true"` and use current Node 24-capable action majors (`checkout@v6`, `setup-node@v6`, `setup-python@v6`, `upload-artifact@v7`, `pnpm/action-setup@v5`), clearing the previously noted Node 20 deprecation follow-up before GitHub's June 2, 2026 default switch.                                               |
...
- **Resuming work?** Read this file first, then `docs/TODO.md`, then `docs/runbooks/w3.2o-done-confirmation.md` for the latest shipped wave.
- **Continuing execution?** W3.2d repo artifacts are on `main`; operator must enable Fly billing and then run `docs/runbooks/w3.2d-fly-scheduler-deploy.md` to complete live deployment (not autonomously runnable). The autonomous queue through 2026-04-24 is complete again: W3.2f `--dry-run`, W3.2g ADA silent-drop, DB-test hygiene, Playwright option D, the GitHub Actions Node 24 migration, W3.2h `fdi_wdc`, W3.2i `eao_congress`, W3.2j `cds_midwinter`, W3.2k `dentex_algeria`, W3.2l `morocco_dental_expo`, W3.2m `europerio`, W3.2n `amied_congress`, and W3.2o `forum_officine_tn` are all wired. With eleven curated sources on `main`, the next optional lane is the generic fallback parser or another curated-source expansion.
