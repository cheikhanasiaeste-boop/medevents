...
- W3.1 + W3.2a + W3.2b are complete. W3.2 continues as sequenced sub-waves (drift observability → third source → Fly scheduler) per [`docs/TODO.md`](TODO.md) "Now" section. Next wave is W3.2c.
...
| W3.2c — detail-page drift observability + `_diff_event_fields` `None`-rule    | 🟡 Next — seeded detail page yielding zero events must signal, not be silent; also fix GNYDM `raw_title` provenance                                                                                      |
...
- **Continuing execution?** W0+W1, W2, W3.1, W3.2a, and W3.2b are closed. Next concrete action is authoring the W3.2c sub-spec (detail-page drift observability + `_diff_event_fields` `None`-as-clear decision + GNYDM `raw_title` provenance fix). External scheduler wiring (W3.2e, Fly.io scheduled machines) comes after W3.2c + W3.2d.
